...
extension Renderer: MTKViewDelegate {
  ...
  func renderSnake(encoder: MTLRenderCommandEncoder) {
    encoder.setRenderPipelineState(pipelineState)

    store.state.position.forEach({ pos in
      renderSquare(position: pos, color: float4(0,0,0,1), encoder: encoder)
    })
  }
  func renderApple(encoder: MTLRenderCommandEncoder) {
    encoder.setRenderPipelineState(pipelineState)

    renderSquare(position: store.state.applePosition, color: float4(1,0,0,1), encoder: encoder)
  }
  ...
  func draw(in view: MTKView) {
    ...
    renderApple(encoder: renderEncoder)
    renderSnake(encoder: renderEncoder)
